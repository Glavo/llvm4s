...
object JNIUtils {
  ...
  def load(cls: Class[_], libName: String): Unit = {
    load(Class.forName(s"${cls.getPackage.getName}.${cls.getSimpleName}_${platform.replace('-', '_')}")
      .getResource(s"$platform/$prefix$libName$suffix"))
  }

  lazy val load: Unit = {
    load(classOf[NativeLibrary], "LLVM-5.0")
    load(classOf[NativeLibrary], "LTO")
    load(classOf[NativeLibrary], "llvm4s")
  }
